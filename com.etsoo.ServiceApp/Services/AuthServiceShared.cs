...
namespace com.etsoo.ServiceApp.Services
{
        ...
        where U : ICurrentUser, IUserCreator<U>
    {
        ...
        /// <param name="cancellationToken">Cancellation token</param>
        /// <returns>Result</returns>
        public async Task<IActionResult> ChangePasswordAsync(ChangePasswordDto data, CancellationToken cancellationToken = default)
        {
            // Sign the data
            data.Sign = data.SignWith(App.Configuration.AppSecret);

            try
            {
                var api = $"{App.Configuration.ApiUrl}/Auth/ChangePassword";

                using var response = await _clientFactory.CreateClient().PutAsJsonAsync(api, data, ModelJsonSerializerContext.Default.ChangePasswordDto, cancellationToken);
                ...
                catch
                {
                    // Log the response content
                    var content = await response.Content.ReadAsStringAsync(cancellationToken);
                    Logger.LogError("ChangePasswordAsync failed with response: {content}", content);

                    throw;
                }

                return await response.Content.ReadFromJsonAsync(CommonJsonSerializerContext.Default.ActionResult, cancellationToken) ?? ApplicationErrors.NoDataReturned.AsResult();
            }
            catch (Exception ex)
            {
                return LogException(ex);
